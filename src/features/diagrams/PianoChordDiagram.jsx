// PianoChordDiagram — small piano keyboard diagram for a chord voicing

import './diagrams.css';

// One white key (+ optional highlight dot).
function WhiteKey({ x, whiteW, padTop, h, hl }) {
  return (
    <g>
...
// One black key (+ optional highlight dot).
function BlackKey({ cx, blackW, blackH, padTop, hl }) {
  const x = cx - blackW / 2;
  return (
    <g>
      <rect x={x} y={padTop} width={blackW} height={blackH}
            fill="var(--text)" />
      {hl && (
        <circle cx={cx} cy={padTop + blackH - blackW / 2 - 2}
...
export default function PianoChordDiagram({ voicing, name, width = 160, height = 80 }) {
  ...
  return (
    <svg className="diag piano-diag" width={width} height={height} viewBox={`0 0 ${width} ${height}`}>
      ...
      {/* White keys */}
      {whites.map((wk, i) => (
        <WhiteKey key={`w-${i}`} x={padX + i * whiteW} whiteW={whiteW} padTop={padTop} h={h}
                  hl={highlightMap.get(`${wk.name}${wk.octave}`)} />
      ))}

      {/* Black keys (rendered after whites so they sit on top) */}
      {blacks.map((bk, i) => (
        <BlackKey key={`b-${i}`} cx={bk.cx} blackW={blackW} blackH={blackH} padTop={padTop}
                  hl={highlightMap.get(`${bk.name}${bk.octave}`)} />
      ))}
    </svg>
  );
}
