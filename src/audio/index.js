// src/audio/index.js — playback engine (former audio.js / window.HelixAudio).
// Wraps soundfont-player + MusyngKite GM samples (loaded from gleitz/midi-js-soundfonts CDN).
// Instrument players are lazy-loaded and cached.

import Soundfont from 'soundfont-player';
import { noteIndex } from '../theory/index.js';

export const INSTRUMENT_TO_SF = {
...
// AudioContext is created lazily on first use (autoplay policy + SSR/node safety).
function ensureCtx() {
  if (!ctx) {
...
export { ensureCtx as ensureUnlocked };

export function load(instrumentKey) {
  ...
  if (!Soundfont) return Promise.reject(new Error('soundfont-player not available'));
  const c = ensureCtx();
  if (!c) return Promise.reject(new Error('AudioContext unavailable'));
  loading[sf] = Soundfont
...
export function playNote(instrumentKey, midi, opts = {}) {
...
export function playChord(instrumentKey, midiList, opts = {}) {
...
export function voicingToMidis(voicing, openMidis) {
  if (!voicing) return [];
