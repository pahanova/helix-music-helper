...
Энгармоника устроена так: гаммы пишутся побуквенно (7 нот = 7 разных букв; D minor = D E F G A **Bb** C), при плохом написании от выбранного корня берётся энгармонический корень (D# major → Eb major); аккорды следуют знаковой семье своего корня; все сравнения нот на стыках (подсветка, генератор войсингов, in-key) — по pitch class, а не по имени. После любой правки написания запускай `node scripts/check-spelling.js` — прогон всех корней × ладов + golden-кейсы.
...
- Эталонные скриншоты прототипа до переписывания — `docs/screenshots-baseline/`, скриншоты приёмки — `docs/screenshots-rewrite/`, спецификация миграции — `docs/REWRITE-SPEC.md`. Старый no-build код — в git-истории до merge-коммита rewrite (`master@{2026-06-10}`, тег нет; последний коммит прототипа — `ac288c8`).
